...
type ColivingDetailGalleryProps = {
  name: string;
  images: string[];
};

function uniqueImages(raw: string[]): string[] {
...
function buildPreviewImages(all: string[]): string[] {
  return all.slice(0, 5);
}

export function ColivingDetailGallery({ name, images }: ColivingDetailGalleryProps) {
  const allPhotos = useMemo(() => uniqueImages(images), [images]);
  const preview = useMemo(() => buildPreviewImages(allPhotos), [allPhotos]);
  ...
  const openLightbox = useCallback(
    (previewIndex: number) => {
      const src = preview[previewIndex];
      const fullIndex = allPhotos.findIndex((p) => p === src);
  ...
  const activeSrc = lightboxIndex !== null ? allPhotos[lightboxIndex] : null;
  ...
  return (
    <>
      ...
      {portalReady && lightboxIndex !== null && activeSrc
        ? createPortal(
            <div
