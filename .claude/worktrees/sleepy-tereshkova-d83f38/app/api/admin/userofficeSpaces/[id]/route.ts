...
import { getUserOfficeSpaceById } from "@/services/listing-api";
import { getListingSession, respondListing } from "@/services/listing-session";

type Ctx = { params: Promise<{ id: string }> };

/**
 * Read one office space as the owner (Angular `userofficeSpaces/:id` GET).
 */
export async function GET(_request: NextRequest, context: Ctx) {
  ...
  const result = await getUserOfficeSpaceById(id, session.token);
